...
export class ProprietairesController {
  ...
  async create(
    ...
    try {
      ...
      const existingPhone = await this.proprietairesService.findByPhone(
        createProprietaireDto.telephone
      );
      if (existingPhone) {
      ...
      const proprietaire = await this.proprietairesService.create(
